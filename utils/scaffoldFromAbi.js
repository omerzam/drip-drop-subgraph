...
const { abiEvents
} = require('@graphprotocol/graph-cli/src/scaffold/schema')
...
const { addTransactionToMapping, addTransactionToSchema
} = require('./scaffoldHelpers')
...
const updateMustacheConfigFile = ({ contractName, network = 'mainnet', address, startBlock = 0
  ...
  const mustacheConfigPath = path.resolve(__dirname + '/../config/' + `RSK.${network
    }.json`)
...
const getAddressFromConfig = (network, contractName) => {
  const contractAddressesPath = path.resolve(__dirname + '/../config/' + `${network
    }_contracts.json`)
...
const startScaffoldAll = async () => {
  ...
  fs.truncateSync('schema.graphql',
    0)
  const promises = paths.map(async (filePath) => {
    try {
      const relativePath = path.relative(__dirname + '/../', filePath)
      const pathArr = filePath.split('/')
      const contractName = pathArr[pathArr.length - 1
            ].split('.json')[
                0
            ]
      address = getAddressFromConfig(network, contractName)
      if (address) {
        console.log(`found address for contract ${contractName
                }: ${address
                }`)
            } else {
      ...
      fs.writeFile(`./src/${contractName
            }.ts`, tsCode)
      console.log(`adding ${contractName
            } entities to schema.graphsql`)
...
const startScaffoldAbi = async (filepath, address, blockNumber, isGenerateMapping, isGenerateSchema) => {
  if (blockNumber === 0 || blockNumber === undefined || blockNumber === null) {
    console.warn(`WARNING: no proper block number provided for ${address
        } - ${filepath
        }`)
    }
  console.log('starting scaffolding process')
  ...
  if (address === undefined || address === null) {
    console.log(`no address provided for ${contractName
        }, trying to load from json file`)
...
const run = async () => {
  ...
  const { filepath, address, blockNumber, scaffoldAll
    } = options
